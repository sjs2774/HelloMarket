...
<%
	...
	followerList= followDAO.userFollowerList(userNick);
	
	
	String otherNick = null;
	if(request.getParameter(userNick) != null){
		otherNick = request.getParameter(userNick);
	}
	boolean check = false;
                            	...
                            	<section class="left_main profile_right_main">
								    <div class="tab_content follow_tab_content">
								        <div class="my_profile_item_menu">
								            <ul>
								                <li class=""><a class="" href="following.jsp">팔로잉</a></li>
								                <li class="active"><a class="active" href="follower.jsp">팔로워</a></li>
								            </ul>
								        </div>
								        <div class="profile_follow">
								            <ul>
								                <div class="default_message">
								                    <div><span class="txt_color_n">팔로우한 회원이 없습니다.<br></span></div>
								                </div>
								            </ul>
								        </div>
								    </div>
                                        ...
                                        <ul>
                                            <li>
                                                <%
                                                	...
                                                	for(FollowDTO f : followerList){
                                                		int followerIdx = userDAO.getUserIdx(f.getFollowerNick());
                                                		int followerItemCnt = boardDAO.showUserItemCnt(f.getFollowerNick());
                                                		check = followDAO.followingCheck(f.getFollowerNick(), userNick);
;                                                		
                                                %>
                                                		<div><a href="/HelloMarket/mystore/index.jsp?userIdx=<%=followerIdx%>"><img src="https://ccimage.hellomarket.com/web/2019/member/img_apply_profile_4x_0419.png" alt="condor의 프로필이미지"></a></div>
                                                <div><a href="/HelloMarket/mystore/index.jsp?userIdx=<%=followerIdx%>">
                                                        <div><%=f.getFollowerNick() %></div>
                                                        <div><span>상품 &nbsp;</span><span><%=followerItemCnt %></span></div>
                                                    </a></div>
                                                <div>
                                                <%
                                                	if(!check){
                                                %>
                                                		<div class="follower_box"><img src="https://ccimage.hellomarket.com/web/2019/member/ico_plus_14x14_x2.png" alt="체크 이미지" onclick="addFollowing('<%=userNick%>','<%=f.getFollowerNick()%>')"><span>팔로잉</span></div>
                                                
                                                <% 
                                                	}else{
                                                %>
                                                	<div class=""><img src="https://ccimage.hellomarket.com/web/2019/member/ico_white_check_14x14_x2.png" alt="체크 이미지" onclick="cancelFollowing('<%=userNick%>','<%=f.getFollowerNick()%>')"><span>팔로잉</span></div>
